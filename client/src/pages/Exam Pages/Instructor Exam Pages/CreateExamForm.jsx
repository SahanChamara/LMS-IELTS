...
const CreateExamForm = ({ onBack, onExamCreated, initialData, isEdit = false }) => {
  ...
  const validateForm = () => {
    const newErrors = {};
    if (!examData.title.trim()) newErrors.title = 'Exam title is required';
    if (!examData.type) newErrors.type = 'Exam type is required';
    ...
    if (examData.duration < 1) newErrors.duration = 'Duration must be at least 1 minute';
    setErrors(newErrors);
  ...
  // ✅ Fixed: Separate logic for create vs update
  const handleSaveAndContinue = async () => {
    ...
    try {
      if (isEdit) {
        // 🟡 EDIT MODE — Do not call createBasicExamAPI()
        const updatedExam = { ...initialData, ...examData };
        onExamCreated(updatedExam); // you can rename this to onExamUpdated if you want
        showToast("Exam Updated", "Basic exam details updated successfully.", "success");
      } else {
        // 🟢 CREATE MODE — Call API to create a new exam
        const newExam = {
        ...
        showToast("Exam Created", "Basic exam details saved successfully. Now add sections.", "success");
      }
    } catch (error) {
          ...
          <div className="bg-white/80 backdrop-blur-sm border border-blue-200 rounded-lg shadow-md">
            <div className="p-6">
              ...
              <p className="text-gray-600 text-sm mt-1">
                {isEdit
                  ? 'Update the basic information for your IELTS exam'
                  : 'Provide the basic information for your IELTS exam'}
              </p>
            </div>
