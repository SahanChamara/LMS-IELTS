...
const SubmissionList = () => {
  ...
  const fetchSubmissions = async () => {
    try {
      ...
      showToast("Error", "Failed to load submissions. Please try again.", "destructive");
    } finally {
      setLoading(false);
    }
  };
  ...
  const filterSubmissions = () => {
    ...
    if (examFilter !== 'all') {
      filtered = filtered.filter(submission => submission.examId === examFilter);
    }
    ...
    if (dateFilter !== 'all') {
      ...
      if (dateFilter !== 'all') {
        filtered = filtered.filter(submission => 
          new Date(submission.submissionDate) >= filterDate
        );
      }
    }
  ...
  const handleExportData = () => {
    const csvData = filteredSubmissions.map(sub => ({
      ...
      Status: sub.status,
      Score: sub.totalScore ? `${sub.totalScore}/${sub.maxScore}` : 'Not graded',
  ...
  const getStatusBadge = (status) => {
    switch (status) {
      ...
      default:
        return <span className="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium border border-gray-300 bg-gray-50 text-gray-700">Unknown</span>;
    }
  };
  ...
  const uniqueExams = [...new Map(submissions.map(s => [s.examId, { id: s.examId, title: s.examTitle }])).values()];
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 p-6">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex justify-between items-center mb-8">
          <div>
            <h1 className="text-4xl font-bold text-gray-900 mb-2">Exam Submissions</h1>
            ...
            <div className="p-4">
              <p className="text-2xl font-bold text-indigo-600">
                {submissions.filter(s => s.totalScore).length > 0 
                  ? Math.round(submissions.filter(s => s.totalScore).reduce((sum, s) => sum + (s.totalScore / s.maxScore * 100), 0) / submissions.filter(s => s.totalScore).length)
                  : 0}%
              </p>
                  ...
                  <table className="w-full">
                    ...
                    <tbody>
                      {currentSubmissions.map((submission) => (
                        <tr key={submission._id} className="hover:bg-gray-50/50 border-t">
                          ...
                          <td className="px-4 py-2">{submission.sectionTitle}</td>
                          <td className="px-4 py-2">
                            <div className="text-sm">{formatDate(submission.submissionDate)}</div>
                          </td>
