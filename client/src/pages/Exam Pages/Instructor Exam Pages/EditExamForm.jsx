...
const EditExamForm = ({ exam, onBack, onExamUpdated }) => {
  ...
  const handleQuestionsUpdate = (updatedSection) => {
    const updatedSections = examData.sectionsData?.map((s) => 
  ...
  const handlePublish = async () => {
    try {
      ...
      showToast("Exam Published", "Your exam has been published successfully and is now available to students.");
    } catch (error) {
      showToast("Error", "Failed to publish exam. Please try again.", "destructive");
    }
  };
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex items-center mb-8">
          ...
          <div>
            ...
            <p className="text-gray-600">Step 4 of 4: Review your exam and publish</p>
          </div>
        </div>
        ...
        <div className="mb-8">
          <div className="flex items-center justify-between text-sm text-gray-500 mb-2">
            ...
            <span className="text-blue-600 font-medium">Review & Publish</span>
          </div>
        ...
        <div className="bg-white/80 backdrop-blur-sm border border-blue-200 rounded-lg shadow-md mb-6">
          <div className="p-6">
            <div className="flex items-center justify-between">
              <div>
                <h2 className="text-2xl font-semibold text-gray-900">{examData.title}</h2>
                <p className="text-gray-600 text-sm mt-2">{examData.description}</p>
              </div>
              <div className="flex flex-col items-end space-y-2">
                <span className={`
                  inline-flex items-center px-2 py-1 rounded-full text-xs font-medium
                  ${
                    examData.type === 'Reading' ? 'bg-blue-100 text-blue-800' :
                    examData.type === 'Writing' ? 'bg-green-100 text-green-800' :
                    examData.type === 'Listening' ? 'bg-purple-100 text-purple-800' :
          ...
          <div className="p-6 pt-0">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="text-center">
                <p className="text-2xl font-bold text-blue-600">{examData.duration}</p>
                <p className="text-sm text-gray-600">Minutes</p>
              </div>
              <div className="text-center">
                <p className="text-2xl font-bold text-green-600">{examData.sectionsData?.length || 0}</p>
                <p className="text-sm text-gray-600">Sections</p>
              </div>
              <div className="text-center">
                <p className="text-2xl font-bold text-purple-600">{examData.questions || 0}</p>
        ...
        <div className="space-y-4 mb-8">
          {examData.sectionsData?.map((section, index) => (
            <div key={section.id} className="bg-white/80 backdrop-blur-sm border border-gray-200 rounded-lg shadow-md">
              <div className="p-4">
                <h3 className="text-lg font-semibold text-gray-900">Section {index + 1}: {section.title}</h3>
                <p className="text-gray-600 text-sm">
                  {section.duration} minutes • {section.questions?.length || 0} questions
                </p>
              </div>
                ...
                <div className="space-y-2">
                  {section.questions?.map((question, qIndex) => (
                    <div key={question.id} className="flex items-center justify-between p-2 bg-gray-50 rounded">
                      <span className="text-sm">Q{qIndex + 1}: {question.question.substring(0, 50)}...</span>
                      <span className="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium border border-gray-300">
