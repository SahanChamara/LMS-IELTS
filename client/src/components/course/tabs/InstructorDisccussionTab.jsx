...
const InstructorDiscussionsTab = ({ unitId }) => {
  ...
  const handleReply = async (messageId) => {
    ...
    const discussionId = messages.find(m => m.id === messageId)?.discussionId || ''; // Placeholder, adjust logic
    ...
    try {
      await axios.post('/api/discussion/reply', { discussionId, reply: newReply }, { withCredentials: true });
      setMessages(prev => prev.map(m => m.id === replyMessage.id ? { ...m, status: 'sent' } : m));
        ...
        {Object.entries(messageGroups).map(([date, dateMessages]) => (
          <div key={date}>
            ...
            {dateMessages.map((message, index) => (
              <div key={message.id} className={`flex ${message.userId === currentUser.id ? 'justify-end' : 'justify-start'} mt-4`}>
                <div className={`flex max-w-xs lg:max-w-md ${message.userId === currentUser.id ? 'flex-row-reverse' : 'flex-row'}`}>
                  <img src={message.avatar} alt={message.sender} className="w-8 h-8 rounded-full object-cover mr-2" />
                  <div className={`flex flex-col ${message.userId === currentUser.id ? 'items-end' : 'items-start'}`}>
                    {!message.userId === currentUser.id && (
                      <div className="flex items-center space-x-2 mb-1">
                        ...
                        <span className={`text-xs px-2 py-0.5 rounded-full ${message.role === 'instructor' ? 'bg-purple-100 text-purple-800' : 'bg-blue-100 text-blue-800'}`}>
                          {message.role}
                        </span>
                      </div>
                    )}
                    <div className={`px-4 py-2 rounded-2xl ${message.userId === currentUser.id ? 'bg-purple-500 text-white' : 'bg-gray-100 text-gray-900'}`}>
                      <p className="text-sm whitespace-pre-wrap">{message.content}</p>
                    </div>
                    <div className={`flex items-center mt-1 ${message.userId === currentUser.id ? 'flex-row-reverse space-x-reverse' : 'flex-row'}`}>
                      <span className="text-xs text-gray-500">{formatTime(message.timestamp)}</span>
                      {message.userId === currentUser.id && message.status === 'sent' && <Check className="w-3 h-3 text-purple-500 ml-1" />}
                    </div>
                  </div>
                </div>
              </div>
      ...
      <div className="border-t border-gray-200 p-4 bg-gray-50 rounded-b-lg">
        <div className="flex items-end space-x-2">
          ...
          <button
            ...
            className={`p-3 rounded-full ${newReply.trim() ? 'bg-purple-500 text-white hover:bg-purple-600' : 'bg-gray-200 text-gray-400 cursor-not-allowed'}`}
          >
